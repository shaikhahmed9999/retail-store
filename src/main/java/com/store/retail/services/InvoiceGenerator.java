...
public class InvoiceGenerator {
    ...
    public InvoiceGenerator(DiscountCalculator discountCalculator,UserService userService,ItemService itemService) {
    ...
    public InvoiceRPO generateInvoice(OrderRequestRQO orderRequest) {
		...
		double userDiscountPercentage = discountCalculator.calculateUserDiscountPercentage(user.getUserType(), user.getCreatedDate());
		...
		for(OrderItemsRQO orderItem : orderRequest.getItems()) {
			...
			if(item.getIndustry() != Industry.GROCERY) {hunderDiscount+=item.getPrice();}
			items.add(itemRPO.from(item, orderItem.getQuantity()));
			totalBill += discountCalculator.calculateItemDiscount(item,orderItem.getQuantity(),userDiscountPercentage);
            totalDiscountApplied += discountCalculator.calculateDiscountOnItemPrice(item,orderItem.getQuantity(),userDiscountPercentage);
		}
		...
		return new InvoiceRPO(12,user.getUserName(),totalDiscountApplied,items,totalBill);
	}
 }